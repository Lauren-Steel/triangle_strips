...
class Triangle(object):
    ...
    def __init__( self, verts ):
      ...
      Triangle.nextID += 1
      self.flag = 0 #flag to denote if triangle is in strip
...
def findAdjacentTriangles (triangle):
    #creating an array to keep track of how many adjacent triangles each adjacent triangle has
    maxAdjacent = 3
    impossible = maxAdjacent + 1
    adjacentCount = [impossible,impossible,impossible]
    #running through all adjacent triangles
    for i in range (0,len(triangle.adjTris)):
        if len(triangle.adjTris)>1: #checking if we are not at a corner triangle
            adjCounter = 0
            if triangle.adjTris[i].flag==0: #check if this triangle has been visited
                for j in range (0, len(triangle.adjTris[i].adjTris)):   #loop through adjacent triangle's adjacent triangles
                    if triangle.adjTris[i].adjTris[j].flag==0:  #check if it has been visited
                        adjCounter +=1
            if adjCounter == 0: #if no adjacent triangles were found ignore
                adjCounter = impossible
            adjacentCount[i]=adjCounter
        else:
            return 0
    nextUsed = adjacentCount.index(min(adjacentCount))
    return nextUsed #return the index to the adjacent triangle with the least adjacent triangles
...
def buildTristrips( triangles ):
    ...
    #running through all triangles
    while addedTri<len(triangles):
        endLoop = 0
        tempAvailableAdj = 3
        #picking the starting triangle for each strip
        for triangle in triangleSet:
            optionsCount = 0
            if triangle.flag == 1:  #has triangle been added to a strip yet
                continue
            for i in range(0, len(triangle.adjTris)): #loop through adjacent triangles
                if triangle.adjTris[i].flag == 0: #if we haven't used this adjacent triangle increase adjacent options counter
                    optionsCount +=1
            if optionsCount<tempAvailableAdj:   #pick the starting triangle to be the triangle with the least adjacent triangles
                tempAvailableAdj = optionsCount
                firstTriangle = triangle
        currentTriangle = firstTriangle
        previousTriangle = firstTriangle
        while (currentTriangle.nextTri is None) and (currentTriangle.prevTri is None) and (endLoop == 0):
            index = findAdjacentTriangles(currentTriangle)
            currentTriangle.prevTri = previousTriangle #adding current triangle to the strip
            if (currentTriangle.adjTris[index].nextTri is not None) or (currentTriangle.adjTris[index].prevTri is not None):
                currentTriangle.prevTri = previousTriangle #adding final triangle to the strip
                endLoop = 1
                if (currentTriangle.flag == 0):
                    currentTriangle.flag = 1
                addedTri += 1
            else:
                currentTriangle.prevTri = previousTriangle #link current triangle to previous
                currentTriangle.nextTri = currentTriangle.adjTris[index] #link current triangle to next triangle
                previousTriangle = currentTriangle #set previous triangle to current triangle
                triangleSet.remove(currentTriangle) #remove the triangle from set of possible triangles
                currentTriangle.flag = 1 #this triangle is now in a strip
                currentTriangle = currentTriangle.nextTri #set current triangle to next triangle
                addedTri += 1 #increase addedTri count
        currentTriangle.prevTri = previousTriangle

        count += 1 # Increment 'count' every time you *start* a new triStrip.
